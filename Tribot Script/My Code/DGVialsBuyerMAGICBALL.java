...
@ScriptManifest(authors = { "DoomGuard" }, category = "DoomGuard", name = "DG Vial Buyer Teleport", description = "BUY VIAL", version = 1.0)


public class DGVialsBuyerMAGICBALL extends Script implements Painting {
	...
	public boolean nocrap(){
		if(checkfaces() || Inventory.find(995).length == 0){
			return true;
		}
		return false;
	}
	public boolean checkfaces(){
		if(Inventory.find(563).length == 0 || Inventory.find(555).length == 0){
			return true;
		}
		return false;
	...
	public boolean ATBANK(){
		if(Objects.find(5, 24101).length > 0)
			return true;
		return false;
	}
	...
	public boolean isthere(){
		...
		RSObject [] Doorc = Objects.getAt(new RSTile(2902,3428));
		RSObject [] Dooro = Objects.getAt(new RSTile(2901,3428));
		if(Doorc != null)
			if(Doorc[0].isOnScreen())
				return true;
		if(Dooro != null)
			if(Dooro[0].isOnScreen())
				return true;
		return false;
	}
	...
	private boolean TELE(){
		...
		RSInterfaceChild house = Interfaces.get(192, 21);
		try{
			house.click("Cast");
			SleepTill();
		}
	...
	boolean Banking(){
		...
		while(Banking.isPinScreenOpen())
			Banking.inPin();
		while(Inventory.find(995).length == 0 || checkfaces()){
			Banking.depositAll();
			withdrawItem(995,"1k");
			withdrawItem(563,"1");
			withdrawItem(555,"1");
