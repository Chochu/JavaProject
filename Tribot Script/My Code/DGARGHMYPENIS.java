...
@ScriptManifest(authors = { "DoomGuard" }, category = "MoneyMaking", name = "DG XXX", description = "BUY VIAL", version = 1.0)


public class DGARGHMYPENIS extends Script implements Painting {
	...
	public boolean ATBANK(){
		if(Objects.find(8, 8994).length > 0)
			return true;
		return false;
	}
	...
	private void store(){
		...
		RSNPC[] jim = NPCs.findNearest(3882);
		if(jim != null)
			if(!jim[0].isOnScreen()){
				RSNPC target = jim[0];
